...
function RefreshData(){
    ...
    con.BeginExecute('DashBoard',
                     [15],
                     function(response) {
                         DrawTable(response,'#table_stddev',5)
                         DrawTable(response,'#table_busroute',6)
                         DrawTable(response,'#sim_time',0)
                         DrawTable(response,'#csu',1)
                         DrawTable(response,'#tubestart',2)
                         DrawTable(response,'#busstart',4)
                         DrawTable(response,'#totalcredit',7)
                         DrawTable(response,'#table_events',8)
                         DrawTable(response,'#tube_start_stations',9)
                         DrawTable(response,'#tube_end_stations',10)
                         DrawTable(response,'#totalpurchase',11)
                         DrawTable(response,'#totalspend',12)
}
                    );
}

function RefreshStats() {
  con.BeginExecute('MeasureThroughput',
                     [],
                     function(response) {
                         DrawTPSChart(response,'#tps_chart');
                         DrawTable(response,'#swipes',1);
                     }
                    );
}

function RefreshStatusStats() {
  con.BeginExecute('@SystemInformation',
                     ['OVERVIEW'],
                     function(response) {
                         DrawSSTable(response,'#system_status',0);
                     }
                    );
}
...
function DrawSSTable(response, tableName, tableId) {
    try {
        var tables = response.results;
        var hmt = tables[tableId];
        var colcount = hmt.schema.length;
	    ...
	    if (hmt.data[r][1] == 'CLUSTERSTATE' || hmt.data[r][1] == 'UPTIME') {
              tbodyhtml += '<tr>';
              for (var c=0;c<colcount;c++) { // for each column
                  var f = hmt.data[r][c];
