...
function connectToDatabase() {
    ...
    // set the stats interval to run every 1000 milliseconds
    systemStatsIntervalId = setInterval(RefreshStatusStats,1000);
...
function DrawTPSChart(response, someDiv) {
    ...
    var tcount1 = 0;
    var procName = "trandemo.server.CardSwipe";

    for(var r=0;r<table0.data.length;r++){ // for each row
        //var time = table0.data[r][0]/1000;
       
        //if (procName == table0.data[r][1]) {
           tcount1 += table0.data[r][1];
        //  }
        }
    var tps;
   ...
   if (tcount0 == null) {
    ...
    if (tpsVals[tpsVals.length - 1][0] - tpsVals[0][0] > 600000) {
        tpsVals.shift();
    }

    var tpsline = { label: "Card Swipes", data: tpsVals };
    ...
    var options = {
        ...
        xaxis: { mode: "time", timezone: "browser", minTickSize: [300, "second"], ticks: 4 },
...
function DrawTable(response, tableName, selectedRow, tableId) {
    try {
        var tables = response.results;
...
function formatDateAsTime(n) {
    ...
    var s = d.toString().substring(5,29).replace("GMT","");
    return s;
}
